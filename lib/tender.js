...
async function buildQuoteForItem(item, opts, rules, cats) {
  ...
  /* If the tender line matched nothing in the catalog, it is not a system we
     build — a printer, a UPS, an AMC. Back-filling every required category
     would hand back the cheapest server and price it as though we had quoted
     the item, which is how three different line items end up with an
     identical bill of materials. Say we do not stock it instead. */
  if (!solvable.length) {
    return {
      name: item.name,
      ...
      violations: [],
      build: [],
      quote: { lines: [], units: Math.max(1, Number(item.quantity) || 1), subtotal: 0, tax: 0, total: 0,
               cost: 0, gross_margin: 0, gross_margin_pct: 0, lead_days: 0, shortfalls: [] },
...
async function analyse(text, { useAi = true } = {}) {
  ...
  /* Items we do not stock contribute nothing to the total. Counting them
     tells the bid desk how incomplete the estimate is — quoting a partial
     scope as if it were the whole tender is the expensive mistake here. */
  const outside = priced.filter(p => p.outsideCatalog);
  ...
  return {
    ...
    totals: {
      ...
      priced_items: priced.length - outside.length,
      total_items: priced.length,
      outside_catalog: outside.map(p => p.name)
    }
  };
}
